...
def gen_image(X, distributions, epsilons, norm):
    ...
    plt.rcParams["text.latex.preamble"] = (
        r"\usepackage{bm} \usepackage{array} \usepackage{color}"
    )
    n_samples = X.shape[0]
    ...
    for index, ax in enumerate(ax_arr):
        ...
        ax.set_title(
            r"$\varepsilon={0:g}$".format(epsilons[index]), fontdict={"size": 16}
        )

        images = X.copy()

        if epsilons[index] != 0:
            xi, q = distributions[(float(norm), float(epsilons[index]))]
            q_string = ""
        ...
        two_d = np.hstack(
            [three_d[i, :, :].astype(np.uint8) for i in range(three_d.shape[0])]
        )
...
def worst_case_distribution(X, y, epsilon=0.0, qnorm=1, verbose=False):
    ...
    var_s = model.addVars(
        n_samples, vtype=grb.GRB.CONTINUOUS, obj=1.0 / n_samples, name="s"
    )
    var_theta = model.addVars(
        n_features, vtype=grb.GRB.CONTINUOUS, lb=-grb.GRB.INFINITY, name="theta"
    )
    var_p = model.addVars(n_samples, d.size, vtype=grb.GRB.CONTINUOUS, name="p")

    model.addConstrs(
        (
            1
            - y[i] * grb.quicksum(var_theta[j] * X[i, j] for j in range(n_features))
            + grb.quicksum(var_p[i, k] * d[k] for k in range(d.size))
            - grb.quicksum(
                var_p[i, k] * C[k, j] * X[i, j]
                for j in range(n_features)
    ...
    if pnorm == 1:
        slack_var_1 = model.addVars(n_samples, n_features, vtype=grb.GRB.CONTINUOUS)
        model.addConstrs(
            (
    ...
    var_q = model.addVars(n_samples, vtype=grb.GRB.CONTINUOUS, lb=0, ub=1, name="q")
    var_xi = model.addVars(
        n_samples, n_features, vtype=grb.GRB.CONTINUOUS, lb=-grb.GRB.INFINITY, name="xi"
    )
    ...
    obj = (
        grb.quicksum(var_q[i] for i in range(n_samples))
        - grb.quicksum(
            var_q[i] * y[i] * X[i, j] * theta[j]
            for i in range(n_samples)
            for j in range(n_features)
        )
        + grb.quicksum(
            var_xi[i, j] * theta[j] for i in range(n_samples) for j in range(n_features)
        )
    )
...
def main():
    ...
    Z1 = load_svmlight_file(
        "./dataset/MNIST_train_" + str(DIGITS[0]) + ".txt", n_features=n_features
    )
    Z2 = load_svmlight_file(
        "./dataset/MNIST_train_" + str(DIGITS[1]) + ".txt", n_features=n_features
    )
    X = (
    ...
    for epsilon in epsilons:
        ...
        least_favorable_distributions[(float(norm), float(epsilon))] = (xi, q)

        xi, q = worst_case_distribution(X, y, epsilon, norm)
        [xi, q] = make_distribution(xi, q)
        worst_case_distributions[(float(norm), float(epsilon))] = (xi, q)

    fig, arr_ax = gen_image(
        X, least_favorable_distributions, np.insert(epsilons, 0, 0), norm
    )
    fig.savefig("least_inf.pdf", format="pdf", bbox_inches="tight", pad_inches=0.2)

    fig, arr_ax = gen_image(
        X, worst_case_distributions, np.insert(epsilons, 0, 0), norm
    )
    fig.savefig("worst_inf.pdf", format="pdf", bbox_inches="tight", pad_inches=0.2)
